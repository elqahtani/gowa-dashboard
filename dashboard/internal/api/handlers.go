...
func (h *Handlers) listLoggedInDeviceIDs(sourceDeviceID string) ([]string, error) {
	...
	skipped := 0
	for _, d := range devs {
		// Filter berdasar state. Hanya logged_in & connected yang bisa kirim
		// pesan; disconnected akan fail saat handler trigger.
		...
		// Backward compat dengan core versi lama yg punya boolean is_logged_in
